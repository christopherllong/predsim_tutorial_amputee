...
[![Tutorial video thumbnail](https://img.youtube.com/vi/es63PraPR3E/0.jpg)](https://youtu.be/es63PraPR3E)
...
2. **Run simulation.** In `main_comVelocity.py`, let's run the case '0'. You can select which case to run by adjusting the [list cases](https://github.com/christopherllong/predsim_tutorial_amputee/blob/main/main_comVelocity.py#L40). By default, it runs case '0'.
    ...
    - **The code is a work in progress. The simulation should converge, then you can run the section title ["Write Motion Files"](https://github.com/christopherllong/predsim_tutorial_amputee/blob/8f84366ed2bda3cbd28e14a1f19611ee17c89ec7/main_comVelocity.py#L1891) to generate the motion.mot and GRF.mot files.**
...
3.	In the Output field, choose a directory for the results.
4.	Switch to the Analysis tab (rightmost tab), click Add, and select BodyKinematics.
5.	Click Run.

This will generate three output files:
`Hamner_modified_scaled_amp_BodyKinematics_acc_global.sto`
`Hamner_modified_scaled_amp_BodyKinematics_vel_global.sto`
`Hamner_modified_scaled_amp_BodyKinematics_pos_global.sto`

To visualize COM velocity, return to Tools → Plot and load the file:
`Hamner_modified_scaled_amp_BodyKinematics_vel_global.sto`

You can then select the COM velocity variables and plot them in the same way as the joint angles.
